...
def convertStationToID(station_name, stations_csv_file = "stations.csv"):
	try:	
...
def convertIDToStation(station_id, stations_csv_file = "stations.csv"):
	try:
		reader = csv.reader(open(stations_csv_file, 'rb'))
	except IOError as e:
		raise StationError("File " + stations_csv_file + " not found.")
...
def getTrainTimes(**args):
	...
	try:
		...
		print "Error: no from_station specified"
	try:
		to_station = args['to_station']
	except KeyError as e:
		print "Error: no to_station specified"
...
getTrainTimes(to_station = Station(name='Deer Park'), from_station = Station(name='Penn Station'), request_time="04:30")
