...
class Plugin(BasePlugin):
	name = "lirr"
	stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv" #Edit to make user-configurable
	...
	def __init__(self, **args):
		...
		try:
			if isinstance(args['stations_csv_file'], basestring):
				...
				reader = csv.reader(open("/var/root/.pysiriproxy/plugins/stations.csv", 'rb'))
		except KeyError:
			reader = csv.reader(open("/var/root/.pysiriproxy/plugins/stations.csv", 'rb'))
...
###Begin lib methods
def convertStationToID(station_name, stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
	try:	
...
def convertIDToStation(station_id, stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
	try:
		reader = csv.reader(open(stations_csv_file, 'rb'))
	except IOError as e:
		raise StationError("File " + stations_csv_file + " not found.")
...
def getTrainTimes(**args):
	...
	try:
		...
		stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"
...
def getNextTrain(from_station, to_station, stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
...
def nextTrain(from_station_name, to_station_name, stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
...
def trainSchedule(from_station_name, to_station_name, stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
	from_station = Station(name=from_station_name, stations_csv_file = stations_csv_file)
	to_station = Station(name=to_station_name, stations_csv_file = stations_csv_file)
...
def trainSearch(from_station_name, to_station_name, train_time = getTimeCeiling(15*60), am_pm = getAMPM(), date = getTodaysDate(), stations_csv_file = "/var/root/.pysiriproxy/plugins/stations.csv"):
	from_station = Station(name=from_station_name, stations_csv_file = stations_csv_file)
	to_station = Station(name=to_station_name, stations_csv_file = stations_csv_file)
